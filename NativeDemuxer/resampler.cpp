...
resampler::resampler() : src_rate_(48000), dst_rate_(16000), dst_nb_samples_(1), src_ch_layout_(AV_CHANNEL_LAYOUT_MONO), dst_ch_layout_(AV_CHANNEL_LAYOUT_MONO), frame_consumed_(false)
{
    ...
    ASSERT_NON_NEGATIVE(av_samples_alloc_array_and_samples(&dst_data_, &dst_linesize_, dst_ch_layout_.nb_channels, dst_nb_samples_, AV_SAMPLE_FMT_FLT, 0), "Cannot allocate output buffer");
    ASSERT_NON_NEGATIVE(swr_alloc_set_opts2(&resample_context_, &dst_ch_layout_, AV_SAMPLE_FMT_FLT, dst_rate_, &dst_ch_layout_, AV_SAMPLE_FMT_FLT, src_rate_, 0, nullptr), "Cannot create SWR context"); // TODO: check if output should be AV_SAMPLE_FMT_FLT
...
void resampler::write_frame(const uint8_t* frame, const int length, const int timestamp)
{
    ...
    if (dst_nb_samples_ > max_dst_nb_samples_)
    {
        ...
        ASSERT_NON_NEGATIVE(av_samples_alloc(dst_data_, &dst_linesize_, dst_ch_layout_.nb_channels, dst_nb_samples_, AV_SAMPLE_FMT_FLT, 1), "Cannot allocate samples");
