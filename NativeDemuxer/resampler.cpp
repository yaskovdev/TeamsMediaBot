...
resampler::resampler() : src_rate_(48000), src_sample_format_(AV_SAMPLE_FMT_FLT), dst_rate_(16000), dst_sample_format_(AV_SAMPLE_FMT_S16), dst_nb_samples_(1),
    src_ch_layout_(AV_CHANNEL_LAYOUT_MONO), dst_ch_layout_(AV_CHANNEL_LAYOUT_MONO)
{
...
uint8_t* resampler::resample_frame(const uint8_t* src_frame, const int src_length, frame_metadata* dst_metadata)
{
    const int src_nb_samples = src_length / av_get_bytes_per_sample(src_sample_format_);
    ...
    const int samples_per_channel = swr_convert(resample_context_, dst_data_, dst_nb_samples_, &src_frame, src_nb_samples);
    ...
    dst_metadata->type = 1;
    dst_metadata->size = dst_bufsize_;
